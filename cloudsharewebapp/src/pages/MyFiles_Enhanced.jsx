...
const MyFiles = () => {
    ...
    const handleShare = async (file) => {
        if (!file.public && !file.isPublic) {
            toast.error('File must be public to share. Toggle visibility first!');
            return;
        }
                                    ...
                                    <div key={file.id} className="grid grid-cols-5 gap-4 p-3 border-b border-gray-100 hover:bg-gray-50 items-center">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button 
                                                ...
                                                disabled={!file.public && !file.isPublic}
                                                className={`p-1.5 rounded-lg transition ${(file.public || file.isPublic) ? 'hover:bg-orange-100' : 'cursor-not-allowed'}`} 
                                                title={(file.public || file.isPublic) ? "Share Link" : "Make file public to share"}
                                            >
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" className={(file.public || file.isPublic) ? "text-orange-500" : "text-gray-300"}>
